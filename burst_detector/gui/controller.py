...
class Controller(object):
    ...
    def load_recording(self, new_sess=False):
        ...
        while not success:
            try:
                ...
                IndexError and KeyError and FileNotFoundError
            ) as e:  # params file is not in the expected format
                logger.warning(e)
    ...
    def check_create_sim(self):
        ...
        if (
            os.path.isfile(
                os.path.join(self.recording.ks_dir, "automerge", "ae_sim.npy")
            )
            and os.path.isfile(
                os.path.join(self.recording.ks_dir, "automerge", "spk_lat_peak.npy")
            )
            and os.path.isfile(
                os.path.join(self.recording.ks_dir, "automerge", "lat_mean.npy")
            )
            and os.path.isfile(
                os.path.join(self.recording.ks_dir, "automerge", "spk_lab.npy")
            )
        ):

            logger.info("Loading spike latents from file")
            self.ae_sim = np.load(
                os.path.join(self.recording.ks_dir, "automerge", "ae_sim.npy")
            )
            self.spk_lat_peak = np.load(
                os.path.join(self.recording.ks_dir, "automerge", "spk_lat_peak.npy")
            )
            self.lat_mean = np.load(
                os.path.join(self.recording.ks_dir, "automerge", "lat_mean.npy")
            )
            self.spk_lab = np.load(
                os.path.join(self.recording.ks_dir, "automerge", "spk_lab.npy")
            )

            logger.info("Loading autoencoder from file")
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            self.net = bd.autoencoder.CN_AE().to(device)

            if (self.params["model_path"] is not None) and (
                os.path.isfile(self.params["spikes_path"])
            ):
                self.net.load_state_dict(torch.load(self.params["model_path"]))
            elif os.path.isfile(
                os.path.join(self.recording.ks_dir, "automerge", "autoencoder.pt")
            ):
                self.net.load_state_dict(
                    torch.load(
                        os.path.join(
                            self.recording.ks_dir, "automerge", "autoencoder.pt"
                        )
                    )
                )
            self.net.eval()
    ...
    def train_autoencoder(self):
        ...
        if (self.params["spikes_path"] is not None) and (
            os.path.isdir(self.params["spikes_path"])
        ):
            spk_fld = self.params["spikes_path"]
            logger.info(f"Loaded spikes from {spk_fld}")
            ...
            logger.info("Loaded spikes from {spk_fld}")
            self.sim_view.extractProg.setValue(100)
            self.sim_view.extractLbl.setText("Loaded spikes from file.")
        else:
            spk_fld = os.path.join(self.recording.ks_dir, "automerge", "spikes")
            ...
            logger.warning(
                f"No autoencoder snippet folder detected, extracting snippets to {spk_fld}"
            )
            ...
            logger.info("Finished extracting snippets.")
        ...
        if (self.params["model_path"] is not None) and (
            ...
            logger.info(
                f"Loaded autoencoder from {os.path.join(self.recording.ks_dir, "automerge", "autoencoder.pt")}"
            )
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            net = bd.autoencoder.CN_AE().to(device)
            net.load_state_dict(torch.load(self.params["model_path"]))
            net.eval()
            spk_data = bd.autoencoder.SpikeDataset(
                os.path.join(spk_fld, "labels.csv"), spk_fld, ToTensor()
            )
            ...
            logger.info(
                f"Autoencoder saved to {os.path.join(self.recording.ks_dir, "automerge", "autoencoder.pt")}"
            )
            net.eval()

        # calculate spike latents and ae_similarity
        self.ae_sim, self.spk_lat_peak, self.lat_mean, self.spk_lab = (
            bd.stages.calc_ae_sim(
                ...
                [
                    (
                        (i in self.recording.counts)
                        and (self.recording.counts[i] > self.params["min_spikes"])
                    )
                    for i in range(self.recording.n_clust)
                ],
                do_shft=self.params["ae_shft"],
        ...
        # save things
        np.save(
            os.path.join(self.recording.ks_dir, "automerge", "ae_sim.npy"), self.ae_sim
        )
        np.save(
            os.path.join(self.recording.ks_dir, "automerge", "spk_lat_peak.npy"),
            self.spk_lat_peak,
    ...
    def create_cluster_view(self):
        self.clust_view = ClusterView(
            self, self.recording.cluster_metrics, self.recording.load_spikes
        )
    ...
    def create_prb_view(self):
        ...
        # create menu
        self.prb_dock._menu.addAction(
            "Set amplitude threshold", self.prb_view.set_amp_frac
        )
        self.prb_dock._menu.addAction("Set max channels", self.prb_view.set_max_chan)
        self.prb_dock.mode_tgl = self.prb_dock._menu.addAction(
            "Toggle amplitude thresholding"
        )
    ...
    def merge(self):
        ...
        self.clust_view.metrics.loc[new_ind, :] = self.clust_view.metrics.loc[
            cl_ind
        ].sum(axis=0)
        self.clust_view.metrics = self.clust_view.metrics.astype(orig_dtype)
        self.clust_view.metrics.loc[new_ind, "cluster_id"] = new_id

        cl_ind = self.clust_view.metrics["cluster_id"].isin(
            cl_list
        )  # updating to match new df length
        ...
        for i in range(len(int_cols)):
            if int_cols[i] not in exclude:
                self.clust_view.metrics.loc[new_ind, int_cols[i]] = int(
                    self.clust_view.metrics.loc[new_ind, int_cols[i]].item()
                    / len(cl_list)
                )
        ...
        for i in range(len(float_cols)):
            if float_cols[i] not in exclude:
                self.clust_view.metrics.loc[new_ind, float_cols[i]] = int(
                    self.clust_view.metrics.loc[new_ind, float_cols[i]].item()
                    / len(cl_list)
                )
        ...
        for i in range(len(string_cols)):
            if string_cols[i] not in exclude:
                self.clust_view.metrics.loc[new_ind, string_cols[i]] = (
                    self.clust_view.metrics.loc[
                        np.nonzero(cl_ind.to_numpy())[0][0], string_cols[i]
                    ]
                )
        ...
        # delete old rows
        self.clust_view.metrics.drop(
            np.nonzero(cl_ind.to_numpy())[0], axis=0, inplace=True
        )
        ...
        self.clust_view.tbl.selectionModel().select(
            self.clust_view.tbl.indexFromItem(self.clust_view.tbl.item(ind, 0)),
            QItemSelectionModel.ClearAndSelect | QItemSelectionModel.Rows,
        )
        self.clust_view.tbl.verticalScrollBar().setValue(
            self.clust_view.tbl.rowViewportPosition(ind)
        )
    ...
    def label_clusters(self, grp, label, cur_label=""):
        ...
        self.clust_view.metrics.loc[
            self.clust_view.metrics["cluster_id"].isin(cl_list), "group"
        ] = label

        if grp != "all" or (grp == "all" and label == "good"):
            self.clust_view.metrics.loc[
                self.clust_view.metrics["cluster_id"].isin(cl_list), "cur_label"
            ] = cur_label
        else:
            self.clust_view.metrics.loc[
                self.clust_view.metrics["cluster_id"].isin(cl_list), "cur_label"
            ] = self.clust_view.filtText
        ...
        status = (
            "Labelled clusters " + str(cl_list)[-1:1] + " as %s" % (label)
            if grp == "selected"
            else "Labelled all as %s" % label
        )
        if cur_label != "":
            status += " (%s)" % cur_label
    ...
    def sel_change(
        self,
    ):  # if remaking graphics layout is too slow, split this into one signal for clearing and one signal for adding
        if self.sim_done:
            self.done_train()
            self.sim_done = False
