...
def extract_noise(data, times, pre_samples=20, post_samples=62, n_chan=385):
    ...
    for i in range(1,times.shape[0]):
        ...
        if ((times[i]-pre_samples) - (times[i-1]+post_samples)) > pre_samples + post_samples:

            # randomly select 10 times and channels in range
            noise_times = np.random.choice(range(int(times[i-1]+post_samples), int(times[i]-pre_samples)), 10, replace=False)
            ...
            for j in range(10):
                ...
                noise[:, ind*snip_len:(ind+1)*snip_len] = np.nan_to_num(noise[:, ind:ind+pre_samples+post_samples])
                ind += 1
...
def calc_metrics(ks_folder, data_filepath, n_chan):
    ...
    num_peaks, num_troughs, wf_durs, spat_decays = calc_wf_shape_metrics(mean_wf, cl_good, channel_pos, 0.2)
    ...
    wf_df = pd.DataFrame({"cluster_id": cl_ids, "num_peaks": num_peaks, "num_troughs": num_troughs, 'wf_dur': wf_durs, 'spat_decays': spat_decays})
...
def calc_sliding_RP_viol(times_multi, cl_good, n_clust, bin_size=0.25, acceptThresh=0.25):
...
def calc_wf_shape_metrics(mean_wf, cl_good, channel_pos, minThreshDetectPeaksTroughs=0.2):
    peak_chans = np.argmax(np.max(np.abs(mean_wf), axis=-1), axis=-1)
    ...
    for i in range(mean_wf.shape[0]):  
        if cl_good[i]:
            ...
            channels_with_same_x = np.squeeze(np.argwhere(np.abs(channel_pos[:,0] - channel_pos[peak_chans[i], 0]) <= 33))
            if channels_with_same_x.shape[0] > 5:
                peak_idx = np.squeeze(np.argwhere(channels_with_same_x == peak_chans[i]))

                if peak_idx > 5:
                    channels_for_decay_fit = channels_with_same_x[peak_idx:peak_idx-5:-1]
                else:
                    channels_for_decay_fit = channels_with_same_x[peak_idx:peak_idx+5]

                spatialDecayPoints = np.max(np.abs(mean_wf[i, channels_for_decay_fit, :]), axis=0)
                estimatedUnitXY = channel_pos[peak_chans[i], :]
                relativePositionsXY = channel_pos[channels_for_decay_fit, :] - estimatedUnitXY
                ...
                spatialDecayFit = np.polyfit(channelDists_relative[indSort], spatialDecayPoints_norm, 1)
...
if __name__ == '__main__':
    calc_metrics(
        r"C:\SGL_DATA\101323_INS7_DT3_rec_g0\catgt_101323_INS7_DT3_rec_g0\101323_INS7_DT3_rec_g0_imec0\imec0_ks2",
        r"C:\SGL_DATA\101323_INS7_DT3_rec_g0\catgt_101323_INS7_DT3_rec_g0\101323_INS7_DT3_rec_g0_imec0\101323_INS7_DT3_rec_g0_tcat.imec0.ap.bin",
        385
                 )
